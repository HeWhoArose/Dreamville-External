...
export const StoryHUDDrawer: React.FC<StoryHUDDrawerProps> = ({ storyId, onSelectFreeformTab }) => {
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-stone-900/90 backdrop-blur border-b border-stone-800 text-stone-200 text-xs sm:text-sm">
        <div className="flex items-center gap-3">
          <button
            ...
            aria-controls="story-hud-drawer-container"
            ...
            aria-label={`Open Character Sheet for ${protagonist.name || 'Protagonist'}, Health ${health.current} of ${health.max}`}
          >
            <span className="font-medium text-stone-100">{protagonist.name || 'Protagonist'}</span>
            <div className="w-20 bg-stone-800 h-2 rounded-full overflow-hidden border border-stone-700" aria-hidden="true">
              <div
            ...
            <span className="text-stone-300 font-mono text-xs">{health.current}/{health.max}</span>
          </button>

          {/* Quick status indicators */}
          {protagonist.injuries?.length > 0 && (
            <span className="hidden md:inline-flex items-center gap-1 text-red-300 bg-red-950/60 px-2 py-0.5 rounded border border-red-900 text-xs">
              <AlertTriangle className="w-3 h-3 text-red-400" /> {protagonist.injuries.length} Injury
            </span>
          )}
          {protagonist.activeEffects?.length > 0 && (
            <span className="hidden md:inline-flex items-center gap-1 text-amber-300 bg-amber-950/60 px-2 py-0.5 rounded border border-amber-900 text-xs">
              <Zap className="w-3 h-3 text-amber-400" /> {protagonist.activeEffects.length} Effect
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <button
            ...
            className="px-2.5 py-1 rounded bg-stone-800 hover:bg-stone-700 text-stone-200 transition-colors flex items-center gap-1 text-xs"
          >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-fadeIn" role="dialog" aria-modal="true" aria-label="Character & World Journal">
          <div id="story-hud-drawer-container" className="w-full max-w-2xl bg-stone-900 border-l border-stone-800 h-full flex flex-col shadow-2xl text-stone-100">
            {/* Drawer Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-stone-800 bg-stone-950/60">
              ...
              <button
                ...
                className="p-2 text-stone-300 hover:text-stone-100 rounded-lg hover:bg-stone-800 transition-colors"
            ...
            {/* Navigation Tabs */}
            <div className="flex overflow-x-auto border-b border-stone-800 bg-stone-950/40 px-4 py-2 gap-1.5" role="tablist" aria-label="Journal Sections">
              {[
