...
async function run() {
  ...
  const ttsRes = await makeRequest('/api/game/sensory/speech', JSON.stringify({
    storyId: 'default_story',
    actorId: 'player_actor_default_story',
    text: 'Verification test'
  }));
  console.log("TTS Success:", ttsRes.success);
  console.log("TTS Audio Result starts with:", ttsRes.audioResult ? ttsRes.audioResult.substring(0, 20) : null);

  console.log("\n--- TEST STT ---");
  const sttRes = await makeRequest('/api/game/sensory/transcribe', JSON.stringify({
    storyId: 'default_story',
    audioBase64: 'UklGRiQAAABXQVZFZm10IBAAAAABAAEAQB8AAIA+AAACABAAZGF0YQAAAAA='
  }));
  console.log("STT Text:", sttRes.text);

  console.log("\n--- TEST EPISTEMIC SECURITY VIA HTTP ---");
  // 1. Set override for mock model
  ...
  if (turnRes.sensoryEvents) {
    ...
    console.log("No sensoryEvents returned!");
  }
}
