...
import { CANONICAL_NAV_ITEMS, CANONICAL_NAV_SECTION_ORDER, getNavigationItemsForSection } from './navigationModel';
...
export const SidebarNav: React.FC<SidebarNavProps> = ({
	...
	<aside
		className={`hidden md:flex flex-col border-r border-[var(--db-border-default)] bg-[var(--db-bg-canvas)] transition-all duration-200 select-none overflow-y-auto ${isCollapsed ? 'w-16' : 'w-60'}`}
		role="navigation"
	...
	>
		<div className="p-3 space-y-5">
			{CANONICAL_NAV_SECTION_ORDER.map((section) => {
				const items = getNavigationItemsForSection(section).filter((item) => item.desktop);
				if (!items.length) return null;
				return (
					<div key={section} className="space-y-1">
						{!isCollapsed && <div className="px-3 py-1 text-[10px] font-semibold tracking-wider text-[var(--db-text-muted)] uppercase">{section}</div>}
						{items.map((item) => {
							...
							return (
								<button
									...
									className={`w-full flex items-center gap-3 px-3 py-2 rounded-[var(--db-radius-md)] text-xs font-medium transition-all cursor-pointer ${selected ? 'bg-[var(--db-surface-purple)] text-[var(--db-purple-300)] border border-[var(--db-purple-500)]/40 shadow-[var(--db-shadow-glow-purple)]' : 'text-[var(--db-text-secondary)] hover:text-[var(--db-text-primary)] hover:bg-[var(--db-bg-raised)] border border-transparent'}`}
